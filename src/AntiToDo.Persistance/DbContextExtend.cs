...
namespace AntiTask.Persistance
{
    public class DbContextExtend : DbContext
    {
        ...
        public DbContextExtend(DbContextOptions<AntiToDoDbContext> options) : base(options) { }

        public DbContextExtend(DbContextOptions<AntiToDoDbContext> options,
            IDateTime datetime,
        ...
        private async Task<List<AuditEntry>> OnBeforeSaveChangesAsync()
        {
            ...
            {
                Audit audit = auditEntry.ToAudit();
                Audits.Add(audit);
            }
